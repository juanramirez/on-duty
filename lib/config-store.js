...
const APP_FILE = new URL('../config/app.js', import.meta.url);
...
export const getConfig = async () => {
    const [doctorsConfig, datesConfig, offsetsConfig, algorithmConfig, pricingConfig, appConfig] = await Promise.all([
        importFresh(DOCTORS_FILE),
        importFresh(DATES_FILE),
        importFresh(OFFSETS_FILE),
        importFresh(ALGORITHM_FILE),
        importFresh(PRICING_FILE),
        importFresh(APP_FILE)
    ]);
...
export const saveConfig = async (config) => {
    ...
    // pricing.js and app.js are server-only — never overwritten via the public API
    const [pricingConfig, appConfig] = await Promise.all([
        importFresh(PRICING_FILE),
        importFresh(APP_FILE)
    ]);
    return {...normalized, pricingTier: pricingConfig.pricingTier, version: appConfig.version ?? ''};
};
